...
$$
\pi(a_t=a_i\mid s_t)=P(a_i\mid s_t) \ \ \ \ i\in\{1, \cdots, k\}
$$
...
$$
\begin{align*}
V_{\pi_\theta}(s_t) &= \mathbb{E}_{( a_t, s_{t+1}, a_{t+1}, \cdots, s_T, a_T) \sim \rho_\theta(\tau\mid s_t)}\left[\sum_{k=1}^Tr_{t+k}\right]\\
&=\mathbb{E}_{a_t \sim \pi_\theta(.\mid  s_t) }\left[\mathbb{E}_{(s_{t+1}, a_{t+1}, \cdots, s_T, a_T) \sim \rho_\theta(\tau\mid s_t, a_t)}\left[\sum_{k=1}^Tr_{t+k}\right]\right]\\
&=\mathbb{E}_{a_t \sim \pi_\theta(.\mid  s_t) }\left[Q_{\pi_\theta}(s_t, a_t)\right]\\
& = \sum_{a_t}\pi_\theta(a_t\mid s_t)Q_{\pi_\theta}(s_t, a_t)
\end{align*}
$$
...
$$
\begin{align*}
Q_{\pi_\theta}(s_t, a_t) &= \mathbb{E}_{(s_{t+1}, a_{t+1}, \cdots, s_T, a_T) \sim \rho_\theta(\tau\mid s_t, a_t)}\left[\sum_{k=1}^Tr_{t+k}\right]\\
&=\mathbb{E}_{s_{t+1}\sim P(s_{t+1}\mid s_t, a_t)}\left[r_{t+1} + \mathbb{E}_{(a_{t+1}, \cdots, s_T, a_T) \sim \rho_\theta(\tau\mid s_{t+1})}\left[\sum_{k=1}^Tr_{t+1+k}\right]\right]\\
...
&= \sum_{a} \left( Q_{\pi_{\theta}}(s, a) \nabla_{\theta} \pi_{\theta}(a\mid s) + \pi_{\theta}(a\mid s) \sum_{s'} P(s' \mid  s, a) \nabla_{\theta} V_{\pi_\theta}(s') \right) \\
...
$$
\begin{align*}
  \nabla_{\theta}V_{\pi_{\theta}}(s) = \psi(s) + \sum_{a}\pi_{\theta}(a\mid s)\sum_{s'}P(s'\mid s, a)\nabla_{\theta}V_{\pi_\theta}(s')
\end{align*}
$$
...
$$
\begin{align*}  
  \nabla_{\theta}V_{\pi_{\theta}}(s) = \psi(s) + \sum_{a}\pi_{\theta}(a\mid s)\sum_{s'}P(s'\mid s, a)\left(\psi(s') + \sum_{a'}\pi_{\theta}(a'\mid s')\sum_{s''}P(s''\mid s', a')\nabla_{\theta}V_{\pi_\theta}(s'')\right)
\end{align*}
$$
...
$$
  \begin{align*}
  \nabla_{\theta}V_{\pi_{\theta}}(s) &= \psi(s) + \sum_{a}\pi_{\theta}(a\mid s)\sum_{s'}P(s'\mid s, a)\bigg(\psi(s') + \sum_{a'}\pi_{\theta}(a'\mid s')\sum_{s''}P(s''\mid s', a')\nabla_{\theta}V_{\pi_\theta}(s'')\bigg)\\
...
$$
  \begin{align*}        
    n(s) &= h(s)+\sum_{s'}n(s')\sum_{a}\pi_\theta(a\mid s')P(s\mid s',a)\\
