...
- **Evaluation**: This is essentially the calculation of the expected return from following $\pi$. Given an initial state distribution $h(s)$, the value of policy $\pi$ can be evaluated as:
...
&\ge \mathbb{E}_{s \sim h(s)} \left[ \mathbb{E}_{a \sim \pi} \left[ Q_\pi(s, a) \right] \right] \\
&= \mathbb{E}_{s \sim h(s)} \left[ V_\pi(s) \right] \\
&= \eta(\pi) \\
&\boxed{\therefore \eta(\pi') \ge \eta(\pi)}
\end{align*}
$$
...
Then by the definition of improvement:
...
This contradicts the assumption that $\pi$ is suboptimal but invariant to improvement. Hence, such a $\pi$ must already be the optimal policy, i.e., $\pi = \pi^*$.
...
In the diagram above, we see two trajectories that are identical except at one decision point, where different actions lead to different rewards.
...
> - **blue**: Another formulation of $\eta(\theta)$ is the expected value of the states sampled from the initial state distribution $h(s_0)$.
> - **red**: We assume that the initial state distribution is the same for the trajectory distribution as well. This assumption allows us to write the expectation under any policy that we desire (as long as the initial state distribution remains same)
> - **green**:
...
\therefore \forall (s,a) \sim P_{\pi_{\theta'}}(s, a), \quad A_{\pi_\theta}(s, a) &> 0 \\
...
- the best trivial case when both the reward and the value for transitioning into a new state are better than before.


This shows that Policy Gradient is Policy Improvement in essence!



In summary, we’ve seen that Policy Gradient can be viewed as a differentiable extension of Policy Iteration. Instead of requiring an exact maximization over actions or states, Policy Gradient moves in the direction of the expected advantage, guaranteeing improvement as long as the new policy is even slightly better than the old one. This connection is crucial in modern reinforcement learning, especially when dealing with large or continuous action spaces where traditional policy iteration becomes infeasible. **Better, not best, is sufficient for improvement!**

---

## References

[^1]: [A Long Peek into Reinforcement Learning by Lilian Weng](https://lilianweng.github.io/posts/2018-02-19-rl-overview/)
[^2]: [Policy Gradient Algorithms by Lilian Weng](https://lilianweng.github.io/posts/2018-04-08-policy-gradient/)
[^3]: [Lectures for UC Berkeley CS 285: Deep Reinforcement Learning](https://www.youtube.com/playlist?list=PL_iWQOsE6TfURIIhCrlt-wj9ByIVpbfGc/)
[^4]: Sutton, R. S., Barto, A. G. (2018). Reinforcement Learning: An Introduction. United Kingdom: MIT Press
